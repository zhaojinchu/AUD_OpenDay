...
Solver.prototype.walkMazeAstar = function (nodes) {
  ...
  while (openSet.length > 0) {
    ...
    if (this.maxSolve && this.maxSolve < max) {
      alert(
        "Solving maze took too long. Please try again or use smaller maze dimensions"
      );
      break;
    }
...
Solver.prototype.walkMaze = function (nodes) {
  ...
  while (this.solved === false) {
    ...
    let directions = Object.keys(node["nswe"]).filter((key) =>
      !node["nswe"][key] ? true : false
    );
...
Solver.prototype.drawAstarSolve = function () {
  ...
  if (hasGates) {
    ...
    ctx.fillRect(
      gateEntry.x * wallSize,
      gateEntry.y * wallSize,
      wallSize,
      wallSize
    );
  }
